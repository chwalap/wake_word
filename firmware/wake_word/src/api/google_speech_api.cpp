...
const char GOOGLE_SPEECH_API[] = "speech.googleapis.com";
...
void google_speech_api::speech_to_text()
{
  ...
  m_client.stop();
  if (auto ret = m_client.connect(GOOGLE_SPEECH_API, 443); ret == 0)
  {
    Serial.printf("Unable to connect to %s: %d\n", GOOGLE_SPEECH_API, ret);
  ...
  synthesizer->say("Dobra, już wiem.");
