...
public:
    ...
    inline static void set(port_t val) __attribute__ ((always_inline)) { if(val) { am_hal_gpio_fastgpio_set(PAD); } else { am_hal_gpio_fastgpio_clr(PAD); } }
    ...
    inline static void fastset(port_ptr_t port, port_t val) __attribute__ ((always_inline)) { set(val); }
